...
class ErrorHelper {
    constructor(modelName, filename) {
        var name = modelName || filename;
        this.modelName = getModelName(name);
    }
    notFound(name) {
        return new errors_1.NotFoundError(`${name} not found!`);
    }
    createError(result, name) {
        if (!result) {
            throw new errors_1.CreateError(`Error creating ${name || this.modelName}`);
        }
    }
